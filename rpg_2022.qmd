...
surfaces_par_culture <- st_read(
  gpkg_file,
  query = "SELECT code_cultu AS `CODE CULTURE`, ROUND(SUM(surf_parc),1) AS `TOTAL (ha)`, COUNT(code_cultu) AS N, ROUND(SUM(surf_parc)/COUNT(code_cultu), 1) AS `MOYENNE (ha)` FROM parcelles_graphiques GROUP BY code_cultu ORDER BY `TOTAL (ha)` DESC;",
  as_tibble = TRUE
)
toc()
