...
gpkg_file <- "DATA/RPG/RPG_2-0__GPKG_LAMB93_FXX_2022-01-01/PARCELLES_GRAPHIQUES.gpkg"
...
## Test de chargement partiel

```{r}
#| label: request_gpkg
#| echo: false

tic(msg = "recherche des cultures de pasteques")
PAS_data <- st_read(
  gpkg_file, 
  query = "select * from parcelles_graphiques where code_cultu = 'PAS';",
  as_tibble = TRUE
)
toc()

# recherche des cultures de pasteques: 17.66 sec elapsed

tic(msg = "recherche des cultures de tournesols")
TRN_data <- st_read(
  gpkg_file, 
  query = "select * from parcelles_graphiques where code_cultu = 'TRN';",
  as_tibble = TRUE
  )
toc()

# recherche des cultures de tournesols: 16.93 sec elapsed
...
tic(msg = "affichage des cultures de tournesol en métropole")
st_geometry(TRN_data) |> 
  ggplot() +
  geom_sf(colour = "salmon")
toc()

# affichage des cultures de tournesol en métropole: 41.08 sec elapsed
...
```{r}
#| label: cultures_dans_gpkg
#| echo: false

file_ref <- "DATA/RPG/REFERENCES/REF_CULTURES_GROUPES_CULTURES_2021.csv"
data_ref = fread(file_ref)

current_data <- data_ref |> filter(CODE_CULTURE %in% code_cultu_gpkg)
...
```{r}
#| label: create_RDS_culture
#| echo: false

get_culture <- function(code_cultu) {

  rds_file <- paste(
    "DATA", "RPG", "RDS", paste0("rpg_",code_cultu,".RDS"), sep = "/"
  )
