...
surfaces_par_culture <- st_read(
  gpkg_file,
  query = "SELECT code_cultu AS `CODE CULTURE`, ROUND(SUM(surf_parc),1) AS `TOTAL (ha)`, COUNT(code_cultu) AS N, ROUND(SUM(surf_parc)/COUNT(code_cultu), 1) AS `MOYENNE (ha)` FROM parcelles_graphiques GROUP BY code_cultu ORDER BY code_cultu;",
  as_tibble = TRUE
)
toc()

data_surfaces <- inner_join(surfaces_par_culture, data_ref, join_by(`CODE CULTURE` == CODE_CULTURE)) |> 
  select(-c(CODE_GROUPE_CULTURE, LIBELLE_GROUPE_CULTURE)) |> 
  rename(c(`LIBELLE CULTURE` = "LIBELLE_CULTURE"))
...
data_surfaces |> 
  formattable(
    align = c("l", "r", "r", "r"),
    list(
      "TOTAL (ha)" = color_tile("transparent", "red"),
      "N" = color_tile("transparent", "green"),
